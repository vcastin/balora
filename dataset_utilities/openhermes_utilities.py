"""
Preprocessing utilities for OpenHermes dataset.
Dataset: https://huggingface.co/datasets/teknium/openhermes
...
def format_openhermes_messages(example: Dict[str, Any]) -> List[Dict[str, str]]:
    """
    Convert OpenHermes example to chat messages format.
...
def preprocess_batch(batch: Dict[str, List[Any]], tokenizer, max_length: int) -> Dict[str, List]:
    """
    Preprocess OpenHermes dataset batch.
    ...
    return preprocess_instruction_dataset(
        ...
        format_messages_fn=format_openhermes_messages
    )
